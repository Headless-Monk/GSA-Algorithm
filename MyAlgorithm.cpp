...
/**
  Realise une evolution complËte sur l'ensemble des solutions (calcul de l'acceleration, calcul de fitness, mise a jour des positions des solutions, ...)
*/
void MyAlgorithm::evolution()
{
    ...
    for(unsigned int iter=0; iter<_setup.get_nb_independant_runs(); iter++)
    {
        ...
        /* recherche des solutuions extr√©males */
        upper_cost();
        lower_cost();
        best_cost_overall();
...
/**
  Cree une nouvelle resolution avec les nouveaux paramËtres modifies
  @param[in] setup - SetUpParams
  @param[in] pbm - Problem
*/
...
/**
  Met ‡ jour la plus haute fitness de la population en fonction des fitness de l'evolution actuelle
*/
...
/**
  Met ‡ jour la plus basse fitness de la population en fonction des fitness de l'evolution actuelle
*/
...
/**
  Verifie si une solution est dans les limites du problËme courant, si non, la solution est replacee avec une postion tiree au hasard ‡ l'interieur des limites
